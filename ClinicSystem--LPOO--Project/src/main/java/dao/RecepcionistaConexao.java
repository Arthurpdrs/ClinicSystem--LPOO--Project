...
public class RecepcionistaConexao extends FuncionarioDAO {
	...
	public boolean addPaciente(String Tipo_sanguineo, String Alergia, String Data_nascimento) throws SQLException {
		try {
			PreparedStatement insert = recepcionista.getConexao().prepareStatement("INSERT INTO Paciente(Tipo_sanguineo, Alergia, Data_nascimento) VALUES(?, ?, ?)");
	...
	public boolean desmarcarConsulta(String Pessoa_CPF, String Medico_Funcionario_Pessoa_CPF)throws SQLException {
		try {
			PreparedStatement delete = recepcionista.getConexao().prepareStatement("DELETE FROM Consulta u WHERE u.Pessoa_CPF = ? AND u.Medico_Funcionario_Pessoa_CPF = ?");
	...
	public boolean marcarConsulta(String Data_consulta, Double Valor, String pago) throws SQLException{
		try {
			PreparedStatement insert = recepcionista.getConexao().prepareStatement("INSERT INTO consulta(Data_consulta, Valor, Pago) VALUES(?, ?, ?)");
	...
	public boolean visualizarPaciente(String Pessoa_CPF) throws SQLException{
		try {
			PreparedStatement select = recepcionista.getConexao().prepareStatement("SELECT * FROM Paciente WHERE Pessoa_CPF = ? ");
